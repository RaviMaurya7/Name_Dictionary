/*This is C++ implemention of Dictionary using trie data structure that support insert search delete
and prifix search in O(lengthof word): Contiributed by Ravi Maurya*/
#include <bits/stdc++.h>
using namespace std;
struct TrieNode
{
...
TrieNode* deleteNode(TrieNode* root, string key, int depth = 0)
{
    ...
    root->children[index] =
            deleteNode(root->children[index], key, depth + 1);
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
        insert(root, keys[i]);
    int input;
    cout<<"\n**************Welcome to Our Name Dictionary*****************\n";
    do
    {
